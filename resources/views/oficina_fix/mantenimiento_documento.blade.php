...
<head>
    ...
    <title>Mantenimientos de documentos</title>
    ...
    <link rel="stylesheet" href="{{ asset('CSS/new/m_documento.css') }}" >

    <script>
        let local_sesion = @json(session('usuario'));
    </script>
    <script src="{{ asset('JS/mantenimientos/m_documento.js') }}"></script>
        ...
        <div class="cuerpo-pagina">
            ...
            <div class="contenido">
                <div class="title">
                    ...
                    <h2>-</h2>
                    <h2 style="font-style: oblique; font-weight: 500;">Tipos de documentos</h2>
                </div>
                <div class="btn_header">
                    <input type="text" class="data_register">
                    <div class="btn_style btn_agregar" onclick="Registrar_tipo()">
                        <p>Agregar</p>
                    </div>
                </div>
                <div class="datatable">
                    <table class="table_maps display responsive" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <input type="text" value="{{ $tipo_documento['tipo_documento'] }}" class="input_custom data_input_{{$tipo_documento['id']}}">
                                </td>
                                <td >
                                    <select class="input_custom data_input_{{$tipo_documento['id']}}">
                                        @if ($tipo_documento['estado'] == 'activo')
                                            <option value="{{ $tipo_documento['estado'] }}" selected>{{ $tipo_documento['estado'] }}</option>
                                ...
                                <td class="btn_section">
                                    <div class="btn_style btn_eliminar data_input_{{$tipo_documento['id']}}" onclick="Eliminar_tipo({{$tipo_documento['id']}})">
                                        Eliminar
                                    </div>
                                    <div class="btn_style btn_guardar data_input_{{$tipo_documento['id']}}" onclick="Guardar_tipo({{$tipo_documento['id']}})">
                                        Guardar
                                    </div>
                                </td>
                            </tr>
...
<script>
    document.querySelectorAll(".cont-items-categorias")[8].classList.add("categoria-active")
</script>
